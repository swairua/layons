...
import { useCompanies, useCustomers } from '@/hooks/useDatabase';
...
export function CreateBOQModal({ open, onOpenChange }: CreateBOQModalProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast.success(`BOQ ${boqNumber} generated`);
      onOpenChange(false);
    } catch (err) {
      console.error('Failed to generate BOQ PDF', err);
      toast.error('Failed to generate BOQ PDF');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Build a detailed BOQ and download a branded PDF.
          </DialogDescription>
        </DialogHeader>
                    ...
                    <TableBody>
                      {section.items.map(row => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell>
                            <Input value={row.unit} onChange={e => updateItem(section.id, row.id, 'unit', e.target.value)} />
                          </TableCell>
                          <TableCell>
