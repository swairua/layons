...
export function DeleteAuditLog() {
  ...
  const { data: auditLogs, isLoading, error } = useQuery({
    queryKey: ['audit_logs', 'delete', currentCompany?.id],
    queryFn: async () => {
  ...
  return (
    <>
      <Card>
        <CardHeader>
          <CardTitle>Deletion Audit Trail</CardTitle>
          <CardDescription>
            Complete history of all deleted records across your company
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
                ...
                {isLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-8 text-slate-500">
                      {auditLogs?.length === 0 ? 'No deletions yet' : 'No matches found'}
                    </TableCell>
                  </TableRow>
                ) : (
                  filteredLogs.map((log) => (
                    <TableRow key={log.id} className="hover:bg-slate-50">
                      ...
                      <TableCell className="text-sm">
                        {format(new Date(log.timestamp || log.created_at), 'MMM dd, yyyy HH:mm')}
                      </TableCell>
          ...
          {(auditLogs?.length || 0) > 0 && (
            <div className="text-sm text-slate-600">
              Showing {filteredLogs.length} of {auditLogs?.length} deletion records
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Dialog open={showDetails} onOpenChange={setShowDetails}>
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Complete information about this deletion
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedLog && (
            <div className="space-y-4">
              ...
              <div className="border-t pt-4">
                <p className="text-xs text-slate-500 uppercase tracking-wide mb-2">Deletion Information</p>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <p className="text-slate-600">Deleted By</p>
                    <p className="font-medium">
                      {selectedLog.details?.deletedBy || users?.[selectedLog.user_id] || 'Unknown'}
                    </p>
                  </div>
                  <div>
              ...
              {selectedLog.details && (
                <div className="border-t pt-4">
                  <p className="text-xs text-slate-500 uppercase tracking-wide mb-2">Additional Details</p>
                  <div className="bg-slate-50 rounded p-3 text-xs font-mono overflow-x-auto">
